...
Production runs `main` commit `420991e` through `pplx-proxy.service` on port **8892**. The attachment and function features below are on **`codex/attachments-function-tools` only** and have **not been deployed**.

| Feature on the development branch | Verified status |
|---|---|
...
| `POST` | `/v1/responses` | Yes | OpenAI Responses API (stream, store, previous_response_id; branch-only function bridge and image input) |
...
| `POST` / `GET` | `/v1/files` | Yes | Branch only: upload/list locally stored files |
| `GET` / `DELETE` | `/v1/files/{id}` | Yes | Branch only: retrieve metadata/delete a stored file |
| `GET` | `/v1/files/{id}/content` | Yes | Branch only: read stored bytes |
...
Function `tool_choice` supports `auto`, `required`, `none`, and `{"type":"function","name":"..."}`. `parallel_tool_calls: false` limits the result to one call. Function parameter schemas, including `strict: true`, are checked locally; this does not provide native constrained generation. Send each result as `function_call_output` with the returned `call_id`. When tool definitions are omitted on a stored continuation, the branch preserves the function context but uses `tool_choice: "none"`; resend the definitions to enable more calls.

### Function loop with the official Python SDK (development branch)

Set `PPLX_PROXY_BASE_URL` to a running instance of the development branch, including `/v1`. The production service on port 8892 does not provide this feature yet.
...
### Image and file input (development branch)
